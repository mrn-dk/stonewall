<script>
  // The conversation, as recorded in the durable log.
  //
  // Note on escaping: the previous implementation built an HTML string for
  // message payloads and rendered it with {@html}, guarded by a hand-written
  // esc(). That path is gone. Role and content are now separate text nodes, so
  // Svelte escapes agent-controlled text by construction and there is no
  // {@html} in the workbench at all — nothing left to guard.
  ...
  /** A message payload may arrive as an object or as a JSON string. */
  ...
  // Only what happened up to the selected turn: the transcript is synced to the
  // timeline, so selecting turn 47 shows the conversation through turn 47.
...
<div class="flex h-full min-h-0 flex-col">
  <div class="flex items-baseline gap-2 px-1 pb-1.5">
    <h2 class="text-muted-foreground text-xs font-medium tracking-wide uppercase">Transcript</h2>
    {#if selectedTurn != null}
      <span class="text-muted-foreground text-xs">through turn {selectedTurn}</span>
    {/if}
  </div>
